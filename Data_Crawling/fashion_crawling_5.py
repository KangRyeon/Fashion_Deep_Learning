# fashion_crawling_5.py : 각 옷 200장 크롤링, 페이지 다운, 각 폴더 생성 자동화
...
for i in range(0, 1):   # 옷의 종류(7번 루프 돎.)
    try: # 중간에 오류나는 것 방지
        ...
        url = "https://www.trunkshow.co.kr/goods/catalog?code=0001000300010006"
        folder_name = "shirt"
        ...
        try:
            post = driver.find_elements_by_xpath('//*[@class="goodsDisplayImageWrap"]')            # <div class="prdimg"> <a href="/암호화된 주소/"> 형식.
        ...
        # 이미지 그림 누를것들 주소 가져옴
        j = 0
        for u in post:          # 모든 goodsDisplayImageWrap 클래스를 가진 요소에서 a태그의 onclick부분 가져옴.
            onclick = u.find_element_by_css_selector('a').get_attribute('onclick')            # 클래스검색 -> 그밑의 태그검색
            #href = post[0].find_element_by_css_selector('a').get_attribute('href') 이것과 같음.
            
            print("주소받아옴[%d] : %s" %(j, onclick))   # href가 아닌 onclick 을 받아와 거기서 숫자만 가져올 것임.
            number = re.findall("\d+", onclick)
            href = "https://www.trunkshow.co.kr/goods/view?no="+number[0]
            arr_href.append(href) # 게시글 주소가 담길 리스트에 넣어줌.
            j = j + 1     # j = 담은 게시글 개수


        count = 0                                                      # count = 이름_번호 에 들어갈 번호 count
